...
[TestClass]
public class FixedIntervalRetryStrategyTests
{
    ...
    {
        RetryPolicy<MockErrorDetectionStrategy> policy = RetryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Fixed Interval, 5 retries");
    ...
    {
        RetryPolicy<MockErrorDetectionStrategy> policy = RetryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Fixed Interval, 5 retries, first retry is fast");
        ...
        catch (Exception)
        {
            ...
            for (int i = 2; i < 5; i++)
            {
                int expectedIntervalInSeconds = TimeSpan.FromSeconds(1).Seconds; // Fixed Interval
    ...
    {
        RetryPolicy<MockErrorDetectionStrategy> policy = RetryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Fixed Interval, 5 retries, no fast retry");
    ...
    public void ExceptionIsThrownWhenFastFirstRetryIsEnabledAndNonTransientExceptionOccurs()
    {
        ...
        {
            Assert.AreEqual(0, (firstTryTime - startTime).Seconds);
            Assert.AreEqual(0, (elapsedTimes[0] - startTime).Seconds);
            for (int i = 1; i < 3; i++)
            {
                Assert.AreEqual(i - 1, (elapsedTimes[i] - startTime).Seconds, "i = " + i.ToString());
            }
    ...
    {
        RetryPolicy<MockErrorDetectionStrategy> policy = RetryManager.GetRetryPolicy<MockErrorDetectionStrategy>("Fixed Interval, 5 retries in Milliseconds");
